...
from crawler.comon import as_chart_array, as_chart
...
def crawl_browser(period: str, chart_type: str, dt: datetime):
    ...
    for child in table_body.select("tr"):
        ...
        if chart_type == "global":
            ...
            data['Title'] = child.select_one("td:nth-child(3) > div > section:nth-child(2) > div > div.font-bold.mb-2").text
            data['Album'] = child.select_one("td:nth-child(3) > div > section:nth-child(2) > div > div.font-bold.mb-2").text
            data['ALBUMIMG'] = child.select_one("td:nth-child(3) > div > section:nth-child(1) > div > div.rounded-full.w-\[70px\].h-\[70px\] > img").get("src")
            ...
            data['FILE_NAME'] = child.select_one("td:nth-child(3) > div > section:nth-child(1) > div > div.rounded-full.w-\[70px\].h-\[70px\] > img").get("src")
            data['ALBUM_NAME'] = child.select_one("td:nth-child(3) > div > section:nth-child(2) > div > div.font-bold.mb-2").text
            data['ARTIST_NAME'] = child.select_one("td:nth-child(3) > div > section:nth-child(2) > div > div.text-sm.text-gray-400.font-bold").text
            ...
            data['RankStatus'] = child.select_one("td:nth-child(2) > div.h-full.text-center.leading-\[30px\].mt-\[7px\].pl-\[10px\] > span").text
            data['RankChange'] = child.select_one("td:nth-child(2) > div.h-full.text-center.leading-\[30px\].mt-\[7px\].pl-\[10px\] > span").text  # 전월대비 (1up)
...
if __name__ == '__main__':
    for n in range(10, 0, -1):
        ...
        chart_array = crawl_browser(MONTH, "global", N_month_ago)
        pprint(chart_array)
    for n in range(130, 0, -1):
        N_months = timedelta(days=TERM * n)
        N_month_ago = TODAY - N_months
        chart_array = crawl_browser(MONTH, "album", N_month_ago)
        pprint(chart_array)
