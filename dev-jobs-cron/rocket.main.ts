...
import { CustomHeader, Response } from "./types/response.types";
...
const emptyQuery =
    "조건에 맞는 결과가 없습니다. 키워드를 바꿔서 검색해 보세요.검색 조건을 저장하면 나중에 추가된 검색 결과를 이메일로 받아볼 수 있습니다.";
...
const shootRocketPunch = async (params: URLSearchParams): Promise<boolean> => {
    return await axios.get(templateURL, { params }).then(
        async (res: Response<"Rocket">) => {
            ...
            if (
                $(".ui.job.items.segment.company-list > div:nth-child(3)").text() ===
                emptyQuery
            ) {
            ...
            companyItems.forEach(async (el: Element) => {
                const title = $(el)
                    .find(".content > .company-name > a > h4")
                    .text()
                    .trim();
                const company_name = removeWhitespace(title);
                const href =
                    $(el).find(".logo.image > a").attr("href")?.replace("/jobs", "") ??
                    "";
                const likes = $(el)
                    .find(".content > .company-name > a.reference-count > span")
                    .text();
                ...
                jobDetail.응답률 =
                    $(el).find(".content > div.company-name > span > div").text() ===
                    "응답률 우수";
                ...
                await axios.get(href, { headers }).then(
                    (response: Response<"HTML">) => {
                        ...
                        imageBox.forEach(async (img: Element, i: number) => {
                            ...
                            src &&
                                (await downloadImage(axios, src, company_name, i).then(
                                    (imgUrl) => {
                    ...
                    (error: AxiosError) => {
                        if (error.code?.startsWith("ECONN")) {
                            console.error(
                                `⌦ can't fetch ${company_name} company page '${error.message}'`
                            );
                        }
                    }
                );
                const jobs = $(el)
                    .find(
                        ".content > div.company-jobs-detail > .job-detail > div > a.job-title"
                    )
                    .toArray();
...
const iterateJobJSON = async () => {
    if (jobDetails.length === 0) {
        const jobDetailJSON = JSON.parse(
            fs.readFileSync("./job-urls.json", { encoding: "utf-8", flag: "r" })
        );
...
async function getDetailOfJobs(url: string, job: RocketJobDetail) {
    await axios.get(url).then(
        async (response: Response<"HTML">) => {
            ...
            const $ = load(html);
            const 주요업무 = removeWhitespace(
                $("div.duty.break > .full-text").text() ?? $("div.duty.break").text()
            );
            ...
            const 복지혜택 = $(
                ".ui.divided.company.info.items > .item > .content"
            ).text();
            const 회사위치 = $(".office.item > .address").text();
            const 채용상세 =
                $(".content.break > .full-text").text() ?? $(".content.break").text();
            ...
            const 아이디 =
                url
                    ...
                    .pop() ?? "아이디";
            const 포지션 = $(
                "body > div.pusher > div.ui.vertical.center.aligned.detail.job-header.header.segment > div > div > h1"
            ).text();
