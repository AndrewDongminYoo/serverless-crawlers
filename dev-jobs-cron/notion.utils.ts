import { AnnotationColor, Annotations, ArrayElement, SelectColor, TextRichTextItem } from './types/notion.types'
import { AxiosError, AxiosInstance, AxiosResponse, ResponseType } from 'axios'
import { MultiSelect, NotionFile, NotionSelect, NotionText, NotionTitle, NotionURL, Numeric, RichText, SelectRequest } from './types/notion.types'
...
export async function downloadImage(axios: AxiosInstance, url: string, company_name?: string, index?: number) {
    ...
    return await axios.get<Stream, AxiosResponse<Stream>>(url, { responseType }).then(
